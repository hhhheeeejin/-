...
# 1. AI 설정
genai.configure(api_key="AIzaSyDOtx8xnAus3UkgN96MMnIjfKqir9W6hzE")
...
# --- 화면 구성 ---
st.set_page_config(page_title="아시아나 채용", page_icon="✈️")
...
with tab2:
    with st.form("apply_form", clear_on_submit=True):
        ...
        q3 = st.radio("새벽 도착 가능?", ["가능", "불가능"])
        submitted = st.form_submit_button("🚀 지금 지원하기")
        
        if submitted:
            if name and phone:
                try:
                    # 💡 핵심: 인증 없이 공개된 시트에 바로 연결
