...
st.markdown("### ✈️ 아시아나 에어포트 실시간 채용 상담")
st.success("안녕하세요! 아시아나 에어포트 채용 담당자 챗봇입니다. 무엇을 도와드릴까요? 😊")

# --- 사이드바: 간편 지원서 (중복 지원 방지 포함) ---
with st.sidebar:
    st.header("📝 간편 지원서 작성")
    with st.form("application_form", clear_on_submit=True):
        ...
        st.divider()
        st.subheader("2. 추가 질문지")
        q1 = st.selectbox("출퇴근 교통방법", ["자차", "대중교통", "셔틀버스"])
        q2 = st.text_input("출퇴근 소요시간")
        q3 = st.radio("오전 05:20까지 도착 가능여부", ["가능", "불가능"])
        
        submitted = st.form_submit_button("지원하기")
        ...
        if submitted:
            if name and phone and address:
                ...
                if is_duplicate:
                    st.warning("이미 동일한 연락처로 지원된 내역이 있습니다!")
                else:
                    new_app = {
                    ...
                    if not os.path.exists(APPLICANT_FILE):
                        ...
                        pd.concat([df_temp, df_app], ignore_index=True).to_excel(APPLICANT_FILE, index=False)
                    st.balloons()
                    st.success(f"{name}님, 지원이 완료되었습니다!")
            else:
                st.error("필수 항목을 모두 입력해 주세요!")

# --- 메인 채팅창 상단 (자주 묻는 질문 버튼) ---
st.write("▼ 궁금하신 내용을 버튼으로 확인해보세요!")
col1, col2, col3 = st.columns(3)
with col1:
    if st.button("💰 급여 안내"): st.session_state.faq_input = "급여 조건에 대해 자세히 알려줘"
with col2:
    if st.button("🚌 셔틀/교통"): st.session_state.faq_input = "셔틀버스 노선과 교통비 지원에 대해 알려줘"
with col3:
    if st.button("⏰ 근무시간"): st.session_state.faq_input = "근무 시간과 스케줄에 대해 알려줘"
...
if "faq_input" in st.session_state:
    ...
    user_input = st.chat_input("채용에 대해 궁금한 점을 물어보세요!")
...
if user_input:
    ...
    if not os.path.exists(QUESTION_LOG_FILE):
        ...
        df_q_old = pd.read_excel(QUESTION_LOG_FILE)
        pd.concat([df_q_old, df_q], ignore_index=True).to_excel(QUESTION_LOG_FILE, index=False)
