...
# 1. AI 설정 (희희진님의 API 키를 직접 설정합니다)
GOOGLE_API_KEY = "AIzaSyDOtx8xnAus3UkgN96MMnIjfKqir9W6hzE"
...
with tab1:
    ...
    if prompt := st.chat_input("질문을 입력하세요!"):
        ...
        with st.chat_message("assistant"):
            context = "너는 아시아나 에어포트 채용 담당자야. 아주 다정하고 자세하게 기초부터 설명해줘."
    ...
    with st.form("apply_form", clear_on_submit=True):
        ...
        if submitted:
            if name and phone and address:
                try:
                    # Secrets에 설정된 서비스 계정 정보를 사용하여 시트에 연결
                    ...
                    st.balloons()
                    st.success(f"🎊 {name}님, 지원서가 성공적으로 접수되었습니다! ✈️")
                except Exception as e:
                    st.error(f"⚠️ 연결 오류 발생! 시트 공유 설정과 Secrets 설정을 다시 확인해주세요.\n(에러내용: {e})")
            else:
                st.error("필수 항목(이름, 연락처, 주소)을 모두 입력해주세요!")
